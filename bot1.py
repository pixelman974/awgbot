...
def traffic_key(message):
    ...
    try:
        ...
        with open('/var/lib/docker/overlay2/19a797cf7f33edc6c47a4f072fab50d1e275ebd642fd203d7bda6928e38d946d/merged/opt/amnezia/awg/clientsTable', 'r') as f:
            data = json.load(f)
...
def info_message(message):
    ...
    bot.send_message(
        ...
        '📱Android - <a href="https://play.google.com/store/apps/details?id=org.amnezia.vpn&pcampaignid=web_share">Скачать из Google Play</a>\n'
        ...
        '<a href="https://docs.amnezia.org/documentation/installing-app-on-linux/">Подробная инструкция по установке на Linux</a>\n'
...
def send_db(message):
    ...
    if os.path.exists(DB_PATH):
        with open(DB_PATH, 'rb') as db_file:
            bot.send_document(message.chat.id, db_file, caption="📦 Вот текущая база данных.")
    else:
        bot.reply_to(message, "❌ База данных не найдена.")

@bot.message_handler(func=lambda message: True)
def unknown_command(message):
    known_commands = ['start', 'key', 'addkey', 'changekey', 'myprofile', 'userkeys', 'users', 'info', 'help', 'path', 'broadcast', 'idkey', 'changeidkey', 'idkeys', 'status', 'c', 'traffickey', 'getdb']
